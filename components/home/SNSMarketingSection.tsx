			...
			<div className="max-w-[1000px] mx-auto">
				...
				<div className="mb-32 pr-[400px]">
					<div className="bg-brand-purple inline-block px-3 py-2 mb-8">
						<h2 className="text-[48px] font-black text-brand-neon uppercase">SNS Marketing</h2>
					</div>
				...
				<div className="space-y-5">
					{services.map((service, index) => (
						<div key={service.id} className="bg-white rounded-[32px] p-10 overflow-hidden">
							<div className="grid grid-cols-1 md:grid-cols-3 gap-5">
								{index % 2 === 0 ? (
									<>
										{/* Images */}
										{service.images.map((image, imgIndex) => (
											<div key={imgIndex} className="flex-1 bg-[#a8a8a8] rounded-2xl border border-black/10 overflow-hidden relative aspect-[30/32]">
												<Image src={image} alt={`${service.title} ${imgIndex + 1}`} fill className="object-cover" />
											</div>
										))}

										{/* Info Card */}
										<div className="flex-1 bg-brand-purple rounded-2xl p-8 flex flex-col justify-between border border-black/10 aspect-[30/32]">
											<p className="text-brand-neon text-[40px] font-black">{service.number}</p>
											<div className="text-white">
												<h3 className="text-[30px] font-black mb-3">{service.title}</h3>
												<p className="text-[16px] font-semibold leading-[1.35] whitespace-pre-line">{service.description}</p>
											</div>
										</div>
									</>
								) : (
